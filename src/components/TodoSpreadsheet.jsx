...
export default function TodoSpreadsheet() {
  ...
  const [newTask, setNewTask] = useState({ task: '', status: 'Not Started', priority: 'Medium', dueDate: '', comments: '' });
  ...
  const addTask = async () => {
    if (newTask.task.trim()) {
      try {
        ...
        setNewTask({ task: '', status: 'Not Started', priority: 'Medium', dueDate: '', comments: '' });
        setError(null);
      } catch (err) {
      ...
      <div className="max-w-7xl mx-auto">
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-green-600 to-green-700 p-4 sm:p-6">
            <h1 className="text-2xl sm:text-3xl font-bold text-white">To-Do List</h1>
            <p className="text-green-100 mt-1 text-sm sm:text-base">Organize your tasks efficiently with DynamoDB</p>
          </div>

          {/* Error Message */}
          {error && (
            <div className="bg-red-50 border-l-4 border-red-500 p-4 m-2 sm:m-4">
              <div className="flex">
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-100 border-b-2 border-gray-300">
                  ...
                  <th className="px-6 py-3 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Comments</th>
                  ...
                  <tr key={task.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <textarea
                        ...
                        className="w-full px-2 py-1 border border-gray-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-green-500 resize-none"
                        rows="2"
                      />
                    </td>
                    <td className="px-6 py-4">
                ...
                <tr className="bg-green-50 border-t-2 border-green-200">
                  ...
                  <td className="px-6 py-4">
                    <textarea
                      ...
                      placeholder="Add notes..."
                      className="w-full px-2 py-1 border border-green-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-green-500 resize-none"
                      rows="2"
                    />
                  </td>
                  <td className="px-6 py-4">
          ...
          {/* Mobile View - Cards */}
          <div className="lg:hidden p-2 sm:p-4 space-y-4">
            {tasks.map((task, index) => (
              <div key={task.id} className="bg-white border-2 border-gray-200 rounded-lg p-4 shadow-sm">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <select
                        value={task.status}
                        onChange={(e) => updateTask(task.id, 'status', e.target.value)}
                        className={`w-full px-2 py-2 rounded-lg font-medium text-sm ${getStatusColor(task.status)} border-0 focus:outline-none focus:ring-2 focus:ring-green-500`}
                      >
                        <option value="Not Started">Not Started</option>
                        <option value="In Progress">In Progress</option>
                        <option value="Completed">Completed</option>
                    ...
                    <div>
                      ...
                      <select
                        value={task.priority}
                        onChange={(e) => updateTask(task.id, 'priority', e.target.value)}
                        className={`w-full px-2 py-2 rounded-lg font-medium text-sm ${getPriorityColor(task.priority)} border-0 focus:outline-none focus:ring-2 focus:ring-green-500`}
                      >
                        <option value="Low">Low</option>
                        <option value="Medium">Medium</option>
                        <option value="High">High</option>
            ...
            <div className="bg-green-50 border-2 border-green-300 rounded-lg p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="font-semibold text-green-700">Add New Task</span>
              </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <select
                      value={newTask.status}
                      onChange={(e) => setNewTask({...newTask, status: e.target.value})}
                      className="w-full px-2 py-2 border border-green-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                    >
                      <option value="Not Started">Not Started</option>
                      <option value="In Progress">In Progress</option>
                      <option value="Completed">Completed</option>
          ...
          {/* Footer Stats */}
          <div className="bg-gray-50 px-4 sm:px-6 py-4 border-t border-gray-200 flex flex-col sm:flex-row justify-between items-center gap-2 sm:gap-0">
